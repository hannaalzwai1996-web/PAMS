...
const TILES: Array<{ key: keyof MatrixSummary; label: string; dot: string }> = [
  ...
  { key: 'auto', label: 'Auto-generated', dot: 'bg-brand-600' },
  { key: 'manual', label: 'Manually edited', dot: 'bg-amber-500' },
    ...
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
      {TILES.map((tile) => (
        <Card key={tile.key} className="flex items-center gap-2.5 px-4 py-3">
          ...
          <div>
            <p className="text-lg font-semibold leading-none text-brand-950 dark:text-white">{summary[tile.key]}</p>
